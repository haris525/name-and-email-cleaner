...
def preprocess_and_remove_names(text):
    """
    Preprocesses the text by cleaning it and removing names, also returns the original
    and cleaned text lengths and the percentage reduction in both words and tokens.
    """
    original_tokens = [token.text for token in nlp(text)]
    original_len = len(original_tokens)
    
    cleaned_text = clean_text(text)
    cleaned_tokens = [token.text for token in nlp(cleaned_text)]
    cleaned_len = len(cleaned_tokens)
    
    no_names_text = remove_names(cleaned_text)
    no_names_tokens = [token.text for token in nlp(no_names_text)]
    no_names_len = len(no_names_tokens)
    
    word_percentage_reduction = 100 * (1 - (len(no_names_text.split()) / len(text.split()) if len(text.split()) > 0 else 0))
    token_percentage_reduction = 100 * (1 - (no_names_len / original_len if original_len > 0 else 0))
    
    return no_names_text, original_len, cleaned_len, word_percentage_reduction, token_percentage_reduction
...
if clean_option:
    cleaned_text, original_len, cleaned_len, word_reduction, token_reduction = preprocess_and_remove_names(user_input)
    st.markdown("## Cleaned Text")
    st.write(cleaned_text)
    st.markdown(f"**Original Length:** {original_len} tokens")
    st.markdown(f"**Cleaned Length:** {cleaned_len} tokens")
    st.markdown(f"**Word Reduction:** {word_reduction:.2f}%")
    st.markdown(f"**Token Reduction:** {token_reduction:.2f}%")
